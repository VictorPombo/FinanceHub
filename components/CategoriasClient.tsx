...
export default function CategoriasClient({ initialData, userId, initialCategories }: Props) {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-zinc-900/90 backdrop-blur-md p-3 border border-zinc-800 shadow-xl rounded-xl text-zinc-200">
          <p className="font-bold text-xs uppercase tracking-wider text-zinc-400 mb-1">{payload[0].name}</p>
  ...
  return (
    <div className="flex flex-col md:flex-row h-full overflow-y-auto md:overflow-hidden bg-[#09090b] text-zinc-100 font-sans">
      
      {/* Table & CRUD Section */}
      <div className="w-full md:w-[60%] flex flex-col h-auto min-h-[450px] md:h-full md:overflow-hidden border-b md:border-b-0 md:border-r border-zinc-800/50 bg-[#09090b]">
        ...
        <div className="p-4 border-b border-zinc-800/60 bg-zinc-900/30 shrink-0">
           ...
           <div className="flex gap-2">
              <input 
                 ...
                 className="flex-1 bg-zinc-900/80 border border-zinc-800 rounded-lg px-4 py-2 text-sm text-zinc-200 placeholder-slate-600 outline-none focus:border-violet-500 transition-colors"
              />
              <button onClick={handleAddCat} className="bg-violet-600 hover:bg-violet-500 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-[0_0_15px_rgba(147,51,234,0.3)] transition-all flex items-center gap-2">
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full text-left border-collapse text-sm">
            <thead className="sticky top-0 z-20">
              <tr>
                <th className="w-[30%] px-4 py-3 bg-[#060b18] border-b border-zinc-800/60 text-xs text-zinc-400 font-bold uppercase tracking-wider">Custo / Categoria</th>
                <th className="text-right px-4 py-3 bg-[#060b18] border-b border-zinc-800/60 text-xs text-zinc-400 font-bold uppercase tracking-wider">Entradas</th>
                <th className="text-right px-4 py-3 bg-[#060b18] border-b border-zinc-800/60 text-xs text-zinc-400 font-bold uppercase tracking-wider">Saídas</th>
                <th className="text-center px-4 py-3 bg-[#060b18] border-b border-zinc-800/60 text-xs text-zinc-400 font-bold uppercase tracking-wider">%</th>
                <th className="text-right px-4 py-3 bg-[#060b18] border-b border-zinc-800/60 w-[50px]"></th>
              </tr>
            </thead>
            <tbody>
              {metrics.map((row, i) => {
                ...
                return (
                  <tr key={row.categoria} className="hover:bg-zinc-800/20 border-b border-zinc-800/40 transition-colors group">
                    <td className="font-semibold text-zinc-200 px-4 py-3.5">
                      <div className="flex items-center gap-2.5 shrink-0">
                        <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: pieData.find(p => p.name === row.categoria) ? COLORS[i % COLORS.length] : '#27272a' }}></div>
                        {row.categoria}
                      </div>
                    </td>
                    ...
                    <td className="text-center font-mono text-xs text-zinc-500 bg-zinc-900/20 px-4 py-3">{row.perc.toFixed(1)}%</td>
                    <td className="text-right px-4 py-3">
                        {dbCat && (
                          <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                             {/* Botão de Zerar Custos daquela categoria */}
                             <button onClick={() => handleZerarCustos(dbCat.nome)} title="Zerar todos os lançamentos desta categoria" className="p-1.5 text-zinc-500 hover:text-amber-400 hover:bg-amber-950/40 rounded transition-all">
                                <Eraser className="w-3.5 h-3.5" />
                             </button>
                             {/* Botão de excluir categoria inteira */}
                             <button onClick={() => handleDeleteCat(dbCat.id, dbCat.nome)} title="Excluir Categoria" className="p-1.5 text-zinc-500 hover:text-red-400 hover:bg-red-950/40 rounded transition-all">
      ...
      {/* Analytics Section */}
      <div className="w-full md:w-[40%] h-auto min-h-[450px] md:h-full flex flex-col p-6 lg:p-8 bg-gradient-to-br from-[#09090b] to-[#0f172a] shadow-inner relative">
        <div className="flex-1 w-full bg-[#060b18] border border-zinc-800/60 rounded-[2rem] p-6 shadow-2xl flex flex-col relative overflow-hidden backdrop-blur-xl">
          
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-violet-600 via-emerald-500 to-rose-500 opacity-50"></div>
          
          <h2 className="text-xs font-black text-zinc-400 mb-6 shrink-0 uppercase tracking-widest text-center">
          ...
          {pieData.length > 0 ? (
            ...
            <div className="h-full w-full flex items-center justify-center text-zinc-500 italic font-medium text-sm">
