...
import { 
  ...
  ShoppingCart, ShieldAlert, Award, Bot, PencilLine, FileSpreadsheet,
  Globe
} from "lucide-react";
    ...
    <div className="flex flex-col h-full bg-[#020617] overflow-y-auto w-full">
      <div className="bg-gradient-to-r from-[#0F172A] to-[#1E293B] p-6 pb-4 w-full shrink-0 border-b border-[#3B82F6]/30 shadow-lg relative z-20">
        ...
        <div className="flex items-center gap-2 mt-6 overflow-x-auto pb-2 scrollbar-none">
          <button onClick={() => setActiveTab("Geral")} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-colors ${activeTab === 'Geral' ? 'bg-blue-600/20 text-blue-400 border border-blue-500/50' : 'text-slate-400 bg-slate-900/50 border border-transparent hover:text-slate-200'}`}>
            <Globe className="w-4 h-4" /> Visão Geral
          </button>
