  ...
  return (
    <div className="flex flex-col gap-3 mb-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-sm font-black text-slate-200 tracking-tight">Finn <span className="text-indigo-400">Insights</span></h2>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {alertas.map((alerta) => {
          let borderColor = "border-slate-800";
          let bgGradient = "from-slate-900 to-slate-900/80";
          let icon = <AlertTriangle className="w-5 h-5 text-slate-400" />;
          ...
          return (
            <div key={alerta.id} className={`p-4 rounded-xl border ${borderColor} bg-gradient-to-br ${bgGradient} flex flex-col justify-between shadow-lg relative overflow-hidden group`}>
               <div className="flex items-start gap-3">
                  <div className={`p-2 rounded-lg bg-slate-900/50 border ${borderColor} shrink-0`}>
                     {icon}
                  </div>
                  <div>
                    <h3 className={`font-black text-xs uppercase tracking-wider mb-1 ${textColor}`}>{alerta.titulo}</h3>
                    <p className="text-xs text-slate-300 font-medium leading-relaxed mb-4">{alerta.descricao}</p>
                  </div>
               </div>
               ...
               <div className="flex items-center justify-between mt-auto pt-2 border-t border-slate-800/60">
                 {alerta.valor_sugerido ? (
                   ...
                   <span className="text-xs text-slate-500 italic">Análise diária</span>
                 )}
                 <button 
                  onClick={() => handleMarcarResolvido(alerta.id)}
                  className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-slate-400 hover:text-white font-bold transition-colors">
